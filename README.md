...
| `include_shade_switches` | false | Expose motorized shades as separate Open, Close, and Stop buttons instead of a position slider. Each button is only added when the shade actually supports that action (Stop requires the Bond `Hold` action). |
